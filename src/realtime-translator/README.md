...
Windows 11 用の Electron コンパニオン アプリです。Teams のプラグインや Teams app では
ありません。システム ループバック（既定のスピーカー、PC の全システム出力）と別途選択した
...
会話終了後に 3 種類の MP3 を保存し、**一時録音を削除**してから次の会話を開始します。
未保存の一時録音は、アプリを終了して次回起動した時点で削除されます。

## 音声とデータの取り扱い

ループバックには Teams 以外の通知音・他アプリの音声・機密情報が含まれる可能性があります。
