...
4. **会話を開始** を押します。左ペインへ相手の英語原文と日本語訳、右ペインへ自分の
   日本語原文と英語訳が、時間をそろえた連続 transcript として表示されます。
5. 終了時は **会話を終了** を押します。Realtime session の残りを flush してから、
   必要なら **日本語で会話を要約** と **日本語で Next Actions を作成** を選択し、
   **会話音声を保存**を押します。
6. 必要な保存を確認したら **一時録音を削除** を押します。削除後に次の会話を開始できます。
